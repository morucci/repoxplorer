...
class RefsCleaner():
    ...
    def clean_tags(self, base_id):
        ...
        if ids:
            self.logs.info("%s tags are removed from %s" % (
                len(ids), base_id))
            self.t.del_tags(ids)
    ...
    def remove_from_seen_refs(self, ref_id):
        ...
        id_elems = ref_id.split(':')
        base_id = "%s:%s" % (id_elems[0], id_elems[1])
        # If the base_id of the ref_id no longer in base_ids
        # remove associated tags
        if base_id not in self.base_ids:
            logger.info("Repo %s no longer referenced clean tags" % base_id)
