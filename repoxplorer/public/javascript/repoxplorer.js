...
function init_menu() {
    // TODO better way to check whether keycloak is configured?
        ...
        console.log('keycloak cannot be initialized, skipping.');
        _init_menu();
    });
}
...
function user_page_init() {
  ...
  function submit_prepare_data(event) {
      ...
      if (keycloak.authenticated) {
          keycloak.updateToken(30).success(submit_send_data());
      } else {
          submit_send_data();
      }
  }

  function submit_send_data() {
