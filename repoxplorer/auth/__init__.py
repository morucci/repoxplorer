...
class OpenIDConnectEngine(BaseAuthEngine):
    ...
    def authorize(self, request, uid=None):
        ...
        try:
            # TODO allow setting intended audience in config to avoid
            # replay attack
        ...
        if uid and uid == claims['preferred_username']:
            return uid
        if uid and uid != token['preferred_username']:
            raise UnauthorizedException("Admin action only: %s/%s" % (uid, token))
        raise UnauthorizedException('unauthorized')
