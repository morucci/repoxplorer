...
class EProjects(object):
    ...
    def __init__(self, version, connector=None):
    ...
    def delete_all(self):
        def gen(docs):
            for doc in docs:
                ...
                yield d
        bulk(self.es,
             gen(self.get_all(set_version=True, neg=True)))
        self.es.indices.refresh(index=self.index)
    ...
    def get_all(self, set_version=None, neg=None):
        if not set_version:
            query = {
                'query': {
                    'match_all': {}
    ...
    def get_by_attr_match(self, attribute, value):
        ...
        body = {
            "query": {
                "bool": {
                    "filter": {'term': {'version': self.version}},
    ...
    def get_by_nested_attr_match(self, attribute, values):
...
class Projects(YAMLDefinition):
    ...
    def get_projects(self):
        projects = {}
        for project in list(self.eprojects.get_all(set_version=True)):
        ...
        for _, details in projects.items():
            for ref in details['refs']:
                for tag in ref.get('tags', []):
                    tags.setdefault(tag, {'refs': []})
                    tags[tag]['refs'].append(ref)
        return tags

    def get_gitweb_link(self, fullrid):
        ret = self.eprojects.get_by_nested_attr_match('fullrid', fullrid)
    ...
    def get_projects_from_references(self, fullrids):
        ret = self.eprojects.get_by_nested_attr_match('fullrid', fullrids)
        projects = set([p['name'] for p in ret[2]])
        return projects
