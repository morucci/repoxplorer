...
class Users(YAMLDefinition):
    ...
    def _flatten(self):
        """ Here we convert data to be loaded by EL
        """
        idents = {}
        for iid, id_data in self.idents.items():
            emails = []
            idents[iid] = {
                'name': id_data['name'],
                'default_email': id_data['default-email'],
                'emails': emails,
            }
            for email, email_data in id_data['emails'].items():
                groups = []
                ...
                for group, group_data in email_data.get('groups', {}).items():
                    if group_data:
    ...
    def get_idents(self):
        idents = {}
        for _ident in self.get_all():
            iid = _ident['_id']
            ident = self.backend_convert_ident(_ident['_source'])
            idents[iid] = ident
        return idents
    ...
    def backend_convert_ident(self, ident):
        ...
        data['default-email'] = ident['default_email']
        data['emails'] = {}
